...
import com.dogpals.forum.config.KafkaProperties;
...
class DogPalsForumKafkaResourceIT {
    ...
    void setup() {
        ...
        DogPalsForumKafkaResource kafkaResource = new DogPalsForumKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dog-pals-forum-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/dog-pals-forum-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
