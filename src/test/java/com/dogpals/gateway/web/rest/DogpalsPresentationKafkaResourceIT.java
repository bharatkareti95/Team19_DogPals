...
import com.dogpals.gateway.config.KafkaProperties;
...
class DogpalsPresentationKafkaResourceIT {
    ...
    void setup() {
        ...
        DogpalsPresentationKafkaResource kafkaResource = new DogpalsPresentationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dogpals-presentation-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/dogpals-presentation-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
