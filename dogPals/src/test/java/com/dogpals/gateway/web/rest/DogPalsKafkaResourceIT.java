...
class DogPalsKafkaResourceIT {
    ...
    void setup() {
        ...
        DogPalsKafkaResource kafkaResource = new DogPalsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dog-pals-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/dog-pals-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
