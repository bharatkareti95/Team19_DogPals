...
import com.dogpals.training.config.KafkaProperties;
...
class DogPalsTrainingKafkaResourceIT {
    ...
    void setup() {
        ...
        DogPalsTrainingKafkaResource kafkaResource = new DogPalsTrainingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dog-pals-training-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/dog-pals-training-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
